...
if [ ! -e /opt/homebrew/bin/brew ]; then
    ...
    for formula in ${brew_formulae[@]}; do
        NONINTERACTIVE=1 HOMEBREW_NO_INSTALL_CLEANUP=1 /opt/homebrew/bin/brew install --quiet ${formula} 2> /dev/null
    done

    for cask in ${brew_casks[@]}; do
        NONINTERACTIVE=1 HOMEBREW_NO_INSTALL_CLEANUP=1 /opt/homebrew/bin/brew install --quiet --cask ${cask} 2> /dev/null
    done
